from ..lec import LEC
from encoding import AAG, GatesAndInputs
...
class AAGLEC(LEC):
    ...
    def _shift_nodes_of_aag(self):
        ...
        for i in range(len(right_scheme_and_gates)):
            right_scheme_and_gates[i][0] += shift
            for j in range(1, 3):
                right_scheme_and_gates[i][j] = AAGLEC._get_and_gate_value_from_input_to_substitute(
                    inputs_to_substitute,
                    right_scheme_and_gates[i][j],
                    shift
                )

        return max_vertex, n_of_and_gates, n_of_inputs, n_of_outputs, right_scheme_outputs, right_scheme_and_gates
    ...
    def _create_xors_by_two_outputs(current_max_node_number, outputs_1, outputs_2):
        ...
        return miter_and_gates, last_layer_and_xor_gates
    ...
    def _merge_schemes_till_xor(self) -> GatesAndInputs:
        ...
        miter_and_gates, last_layer_and_miter_gates = AAGLEC._create_xors_by_two_outputs(
        ...
        aag_components = (
            ...
            left_aag_data.and_gates() + and_gates_2 + miter_and_gates
        )
        ...
        self._max_literal_for_subst = node_number_until_xor
    ...
    def get_handmade_miter(self) -> AAG:
        if self._miter is None:
            ...
            self._miter = AAG(from_gates_and_inputs=(inputs, [current_max_node_number * 2 + 1], miter_and_gates))
