...
class Reducer:
    ...
    # the function counts depth in topological order (starting from inputs)
    def _count_depth(self) -> Mapping[int, int]:
    ...
    def _find_equivalent_nodes(
        ...
        answer, _, _ = analyze_result(result, 'checking if nodes are functionally equivalent', print_output=False)
        ...
        answer, _, _ = analyze_result(result, 'checking if nodes are functionally inequivalent', print_output=False)
        if answer == "UNSAT":
    ...
    def _from_graph_to_aag(self):
        ...
        self._aag._gates_and_inputs = [
            inputs,
            outputs,
            and_gates
        ]
        self._aag._constraints = constraints

    # the function merges equivalent nodes in graph deleting redundant nodes
    # conflict_limit is parameter passed to kissat
    def _merge_equivalent(self, conflict_limit: int = 10000):
        depth = self._count_depth()
        ...
        for hash_equivalence_class in hash_equivalence_classes:
            ...
            for node1_num in range(hash_equivalence_class_len):
                for node2_num in range(node1_num + 1, hash_equivalence_class_len):
                    ...
                    if node1 in self._inputs or node2 in self._inputs:
                        continue
                    if node1 != node2 and not equivalence_classes.belongs(node1, node2):
                        self._find_equivalent_nodes(
                            node1, node2, subgraph_constraints, equivalence_classes, conflict_limit
                        )
        ...
        for _, equivalence_class in equivalence_classes.get_equivalence_classes().items():
            ...
            for other_node_num in other_nodes:
                ...
                Normalize.replace_constraints(self._graph, other_node_num, leader, negation)
                del self._graph[other_node_num]

    # the function reduces graph
    # conflict_limit is parameter passed to kissat
    def reduce(self, conflict_limit: int = 10000):
        self._merge_equivalent(conflict_limit=conflict_limit)
        self._from_graph_to_aag()
